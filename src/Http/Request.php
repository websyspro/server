  ...
  class Request
	{
		public static function getBody(
			...
			if(Request::equestMethod() === RequestMethod::POST->value){
				if(in_array(Request::contentType(), [
					ContentType::ApplicationJSON->value,
					ContentType::MultipartFormData->value,
					ContentType::MultipartFormDataUrlencoded->value,					
				])){
					if(Request::contentType() !== ContentType::ApplicationJSON->value){
						return $_POST;
					}

					return json_decode(
						Request::getPhpInput(), true
					);
				}
			}

			return Request::contentFromFile(RequestType::BODY);
		}
      ...
      array $requestEndpoint = [],
		...
		public static function getFiles(
			...
			return (
				Request::equestMethod() !== RequestMethod::POST->value
					? Request::contentFromFile($requestType)
					: Request::contentPostFile()
			);
		}
        ...
        foreach( static::contentLoadFileList() as $buffer ) {
          ...
          $data[$cursor][] = $buffer;
				...
				foreach( Util::Mapper($data, fn(array $groupBuffer) => array_slice($groupBuffer, 1)) as $contextBuffers ){
					...
					$contextDetailsName = Request::extractName($contextDetails);
					$contextDetailsFile = Request::extractFile($contextDetails);
					$contentSize = Request::extractSize($contextBuffers);
					$contentBody = Request::extractBody($contextBuffers);
					$contentType = Request::extractType($contextType);
				...
				return $content;
      } elseif ( Request::contentType() === ContentType::MultipartFormDataUrlencoded->value ) {
        parse_str( Request::getPhpInput(), $content );
        return $content;
      } elseif ( Request::contentType() === ContentType::ApplicationJSON->value ) {
        return json_decode( Request::getPhpInput(), true );
      }
